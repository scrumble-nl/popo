...
This package only contains a single class (BasePopo) which is intended to be extended by every Popo we create. This 
BasePopo makes it possible for all Popo's to be returned from a controller function and allow it to be transformed into an array.
...
when returning the Popo from a controller function, it is therefore required you mark visible attributes as public.
